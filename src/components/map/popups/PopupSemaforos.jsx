/**
 * Popup solo para markers de capa SEMAFOROS.
 */
import React from 'react';

export default function PopupSemaforos({ feature, onClose }) {
  const p = feature?.properties ?? {};
  const title = p.nombre ?? p.codigo ?? p.nodo_nombre ?? p.direccion ?? p.node_id_externo ?? 'Semáforo';
    ...
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[1002]">
      <div className="pointer-events-auto bg-white/95 backdrop-blur rounded-2xl shadow-2xl w-[min(28rem,92vw)] max-h-[75vh] overflow-hidden border border-slate-200 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 text-sm">
          {p.codigo && <p><span className="text-slate-500">Código:</span> {p.codigo}</p>}
          {p.estado_operativo && <p><span className="text-slate-500">Estado:</span> {p.estado_operativo}</p>}
          {p.plan_semaforico && <p><span className="text-slate-500">Plan:</span> {p.plan_semaforico}</p>}
          {p.origen && <p><span className="text-slate-500">Origen:</span> {p.origen}</p>}
